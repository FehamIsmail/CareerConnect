...
export function SideNav() {
    ...
    const [accountNavigation, setAccountNavigation] = useState([
        {name: 'Profile', href: '/user/profile', icon: UserCircleIcon, current: false},
        ...(user === 'STUDENT'
    ...
    const [jobNavigation, setJobNavigation] = useState([
        ...(user === 'EMPLOYER'
            ? [{name: "Create Job", href: "/job/create", icon: BriefcaseIcon, current: false}]
            : []),
        ...(user === 'STUDENT'
    ...
    const updateCurrentNav = () => {
        ...
        let navToUpdate;
        if(navType === 'user')
            navToUpdate = accountNavigation
        if(navType === 'job')
            navToUpdate = jobNavigation
        if(!navToUpdate || !currentSegment)
            return
        ...
        navToUpdate = navToUpdate.splice(0, navToUpdate.length, ...updatedNavigation);
        if(navType === 'user')
            setAccountNavigation(navToUpdate)
        if(navType === 'job')
            setJobNavigation(navToUpdate)
    }
