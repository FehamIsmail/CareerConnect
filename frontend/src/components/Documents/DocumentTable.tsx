...
export default function DocumentTable(props: DocumentTableProps) {
  ...
  const deleteDocument = (doc: Document) => {
    ...
    const id = doc.id;
    axios
      .delete(`http://localhost:8000${endpoint}${id}/`, {
        headers: {
  ...
  const handleSave = () => {
    ...
    axios
      .put(`http://localhost:8000${endpoint}${id}/`, data, {
        headers: {
          Authorization: `Bearer ${getAccessToken()}`,
        },
      })
  ...
  useEffect(() => {
    documentList.forEach((doc) => {
      if (doc.default) setDefaultDocument(doc);
    });
            ...
            <table className="w-full divide-y divide-gray-300">
              <thead>
                <tr>
                  <th
                    scope="col"
                    className="py-3.5 px-3 text-center w-24 text-sm font-semibold text-gray-900"
                  >
                    Default
                  </th>
                  <th
